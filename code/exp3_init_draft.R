...
# Data gen ----------------------------------------------------------------
  ...
  parms$p       <- 50 # total number of features in the data
  ...
  parms$blck1 <- (tail(parms$blcky, 1)+1):11 # higly correlated
  parms$blck2 <- (tail(parms$blck1,1)+1):21 # correlated
  ...
  parms$S_all   <- list(q1 = (c(parms$blck1, parms$blck2)), # variables in block 1 and 2
                        q2 = (c(4:13, 50:59)))
  parms$stnr    <- 1/10 # signal to noise ratio for alpha coefs
  ...
  parms$beta <- rep(.5, parms$zm_n)
  parms$r2   <- .65 # rsquare for the model (proportion)
